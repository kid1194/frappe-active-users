...
def get_settings():
    ...
    if isinstance(cache, dict) and "refresh_interval" in cache:
        logger.info({"message": "Returning cached settings", "user": user, "data": cache})
        return cache
    ...
    if not settings.is_enabled:
        logger.info({"message": "Plugin not enabled", "user": user, "data": result})
    ...
    if users:
        ...
        logger.info({"message": "Checking users visibility", "listed_users": users, "to_be_hidden": hidden_from_users, "data": result})
        
        if (
            (not hidden_from_users and not in_users) or
            (hidden_from_users and in_users)
        ):
            logger.info({"message": "Hidden from user", "user": user, "data": result})
            frappe.cache().hset(_SETTINGS_CACHE_KEY, user, result)
            return result
    ...
    if not users:
        ...
        if roles:
            ...
            logger.info({"message": "Checking roles visibility", "listed_roles": roles, "to_be_hidden": hidden_from_roles, "data": result})
            
            if (
                (not hidden_from_roles and not in_roles) or
                (hidden_from_roles and in_roles)
            ):
                logger.info({"message": "Hidden from roles", "roles": frappe.get_roles(), "data": result})
                frappe.cache().hset(_SETTINGS_CACHE_KEY, user, result)
                return result
    ...
    frappe.cache().hset(_SETTINGS_CACHE_KEY, user, result)
    logger.info({"message": "Plugin is enabled and visible", "user": user, "data": result})
    return result
...
def get_users():
    ...
    logger.info({"message": "Getting session expiry from system settings", "data": session_expiry})
    ...
    if not session_expiry or not isinstance(session_expiry, str):
        ...
        logger.info({"message": "Getting mobile session expiry from system settings", "data": session_expiry})
    ...
    if session_expiry and isinstance(session_expiry, str):
        ...
        logger.info({"message": "Parsing session expiry", "data": [hours, minutes, seconds]})
    
    end = now()
    start = add_to_date(end, hours=-abs(hours), minutes=-abs(minutes), seconds=-abs(seconds), as_string=True, as_datetime=True)
    logger.info({"message": "Users last active time span", "data": {"start": start, "end": end}})
